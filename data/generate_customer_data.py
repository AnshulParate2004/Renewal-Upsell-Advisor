"""
Generate 2500 customer records for database import
...
for i in range(1, 25001):
    ...
    # High ARR companies tend to have better health scores
    if arr > 200000:
        health_score = random.randint(70, 100)
        risk_score = random.randint(5, 40)
        relationship_score = random.randint(70, 100)
        churn_probability = round(random.uniform(0.05, 0.35), 4)
    elif arr > 100000:
        health_score = random.randint(55, 85)
        risk_score = random.randint(20, 60)
        relationship_score = random.randint(50, 85)
        churn_probability = round(random.uniform(0.25, 0.55), 4)
    else:
        health_score = random.randint(30, 70)
        risk_score = random.randint(40, 90)
        relationship_score = random.randint(25, 70)
        churn_probability = round(random.uniform(0.45, 0.85), 4)
    
    # Sentiment based on health score
    if health_score >= 70:
        sentiment_category = random.choice(["positive", "very_positive"])
        sentiment_score = round(random.uniform(0.4, 1.0), 4)
    elif health_score >= 40:
        sentiment_category = "neutral"
        sentiment_score = round(random.uniform(-0.2, 0.4), 4)
    else:
        sentiment_category = random.choice(["negative", "very_negative"])
        sentiment_score = round(random.uniform(-1.0, -0.2), 4)
    ...
    # Last contact (within last 30 days)
...
# Save to Excel
output_file = "customer_data_25000.xlsx"
