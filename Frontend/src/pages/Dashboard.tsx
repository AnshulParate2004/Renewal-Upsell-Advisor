  ...
  return (
    <div className="p-4 max-w-[1600px] mx-auto h-[calc(100vh-64px)] flex flex-col space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
        {[
          { label: 'Renewals (90d)', value: stats.renewing_count, icon: <Users size={20} />, color: 'text-black' },
          { label: 'Risk Exposure', value: formatCurrency(stats.total_arr_at_risk), icon: <AlertTriangle size={20} />, color: 'text-red-600' },
          { label: 'Forecasted Churn', value: '8.2%', icon: <DollarSign size={20} />, color: 'text-orange-600' },
          { label: 'Upsell Pipeline', value: formatCurrency(stats.upsell_pipeline), icon: <TrendingUp size={20} />, color: 'text-emerald-600' }
        ].map((metric, idx) => (
          <div key={idx} className="p-4 bg-white border-2 border-black flex items-center justify-between shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-transform hover:-translate-y-0.5">
            <div>
            ...
            <div className={`opacity-80 ${metric.color}`}>
              {metric.icon}
            </div>
          </div>
        ))}
      ...
      <AnimatePresence>
        {activeAlert && (
          <motion.div
            ...
            className={`shrink-0 border rounded-md p-2 flex items-center justify-center gap-2 text-xs font-medium ${activeAlert.severity === 'critical' ? 'bg-red-500/10 text-red-500 border-red-500/20' : 'bg-blue-500/10 text-blue-500 border-blue-500/20'}`}
          >
      ...
      <div className="flex-1 min-h-0 border-2 border-black bg-white flex flex-col shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div className="p-3 border-b-2 border-black flex justify-between items-center bg-gray-50 shrink-0">
          <h3 className="text-sm font-black uppercase tracking-wider text-black">Account Risk Analysis</h3>
          <button onClick={fetchData} disabled={isLoading} className="p-1 hover:bg-black/5 rounded transition-colors"><RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} /></button>
        </div>
